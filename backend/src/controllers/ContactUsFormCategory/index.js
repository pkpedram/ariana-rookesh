...
const contactUsFormCategoryController = {
    ...
    getList: {
        ...
        controller: async (req, res, next) => {
            try{
                res.send(await baseResults(ContactUsFormCategory, 'list', req.query, true, []))
            }catch (error){
                if(process.env.NODE_ENV !== 'production'){
                    console.log(error)
                }
        ...
        controller: async (req, res, next) => {
            try{
                ...
                if(contactUsFormCategory){
                            ...
                            return res.send(await baseResults(ContactUsFormCategory, 'id', req.params, false, []))
                          
                           
                }else{
                    res.status(404).send({message: "There is no ContactUsFormCategory with this id"})
                }
            }catch (error){
                if(process.env.NODE_ENV !== 'production'){
                    console.log(error)
        ...
        controller: async (req, res, next) => {
            try{
                ...
                if(contactUsFormCategory){
                    ...
                    const updatedContactUsFormCategory = await contactUsFormCategory.save();
                    if(updatedContactUsFormCategory ){
                        return res.send({result: updatedContactUsFormCategory })
                    }else{
                       return res.status(500).send({message: 'Error in updating ContactUsFormCategory'})
                    }
                }else{
                   return res.status(404).send({message: 'There is no ContactUsFormCategory with this id'})
                }
            }catch (error){
                if(process.env.NODE_ENV !== 'production'){
                    console.log(error)
    ...
    delete: {
        ...
        controller: async (req, res, next) => {
            try{
                const contactUsFormCategory = await ContactUsFormCategory.findById(req.params.id);
                if (contactUsFormCategory) {
