...
const staticAttributeController = {
    ...
    controller: async (req, res, next) => {
      try {
        let staticAttribute = new StaticAttribute({
          ...
          icon: req.files.icon
            ? generateFileName(req.files.icon[0], "staticAttribute")
            : null,
  ...
  getList: {
    ...
    controller: async (req, res, next) => {
      try {
        res.send(
          await baseResults(StaticAttribute, "list", req.query, true, [])
        );
      } catch (error) {
        if (process.env.NODE_ENV !== "production") {
          console.log(error);
  ...
  getDetail: {
    ...
    controller: async (req, res, next) => {
      try {
        ...
        if (staticAttribute) {
          return res.send(
            await baseResults(StaticAttribute, "id", req.params, false, [])
          );
        } else {
          res
            .status(404)
            .send({ message: "There is no StaticAttribute with this id" });
        }
      } catch (error) {
        if (process.env.NODE_ENV !== "production") {
          console.log(error);
  ...
  put: {
    ...
    controller: async (req, res, next) => {
      try {
        ...
        const staticAttribute = await StaticAttribute.findById(id);
        if (staticAttribute) {
          staticAttribute.icon = req.files.icon
            ? generateFileName(req.files.icon[0], "staticAttribute")
            : staticAttribute.icon;
          ...
          if (updatedStaticAttribute) {
            ...
            return res
              .status(500)
              .send({ message: "Error in updating StaticAttribute" });
          }
        } else {
          return res
            .status(404)
