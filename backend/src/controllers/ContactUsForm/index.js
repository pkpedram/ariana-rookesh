...
const contactUsFormController = {
    ...
    getList: {
        ...
        controller: async (req, res, next) => {
            try{
                res.send(await baseResults(ContactUsForm, 'list', req.query, true, ['relatedContactUsFormCategory','relatedProductCategory']))
            }catch (error){
                if(process.env.NODE_ENV !== 'production'){
                    console.log(error)
                }
        ...
        controller: async (req, res, next) => {
            try{
                ...
                if(contactUsForm){
                            ...
                            return res.send(await baseResults(ContactUsForm, 'id', req.params, false, ['relatedContactUsFormCategory','relatedProductCategory']))
                          
                           
                }else{
                    res.status(404).send({message: "There is no ContactUsForm with this id"})
                }
            }catch (error){
                if(process.env.NODE_ENV !== 'production'){
                    console.log(error)
            ...
            try{
                ...
                if(contactUsForm){
                    ...
                    if(updatedContactUsForm ){
                       ...
                       return res.status(500).send({message: 'Error in updating ContactUsForm'})
                    }
                }else{
                   return res.status(404).send({message: 'There is no ContactUsForm with this id'})
                }
            }catch (error){
                if(process.env.NODE_ENV !== 'production'){
                    console.log(error)
