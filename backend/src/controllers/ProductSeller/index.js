...
const productSellerController = {
    ...
    getList: {
        ...
        controller: async (req, res, next) => {
            try{
                res.send(await baseResults(ProductSeller, 'list', req.query, true, ['relatedProduct','relatedSeller']))
            }catch (error){
                if(process.env.NODE_ENV !== 'production'){
                    console.log(error)
                }
        ...
        controller: async (req, res, next) => {
            try{
                ...
                if(productSeller){
                            ...
                            return res.send(await baseResults(ProductSeller, 'id', req.params, false, ['relatedProduct','relatedSeller']))
                          
                           
                }else{
                    res.status(404).send({message: "There is no ProductSeller with this id"})
                }
            }catch (error){
                if(process.env.NODE_ENV !== 'production'){
                    console.log(error)
            ...
            try{
                ...
                if(productSeller){
                    ...
                    if(updatedProductSeller ){
                       ...
                       return res.status(500).send({message: 'Error in updating ProductSeller'})
                    }
                }else{
                   return res.status(404).send({message: 'There is no ProductSeller with this id'})
                }
            }catch (error){
                if(process.env.NODE_ENV !== 'production'){
                    console.log(error)
