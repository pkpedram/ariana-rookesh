...
const generalSettingController = {
  post: {
    ...
    controller: async (req, res, next) => {
      try {
        const foundgeneralSetting = await GeneralSetting.findOne();
        if (foundgeneralSetting) {
          foundgeneralSetting.deleteOne({ _id: foundgeneralSetting?._id });
        }
        let generalSetting = new GeneralSetting({
          ...
          logo: req.files.logo
            ? generateFileName(req.files.logo[0], "generalSetting")
            : req.body?.logo ?? null,
          catalog: req?.files?.catalog
            ? generateFileName(req.files.catalog[0], "generalSetting")
            : req.body?.catalog ?? null,
          isActive: true,
  ...
  getList: {
    ...
    controller: async (req, res, next) => {
      try {
        res.send(
          await baseResults(GeneralSetting, "list", req.query, true, [])
        );
      } catch (error) {
        if (process.env.NODE_ENV !== "production") {
          console.log(error);
  ...
  getDetail: {
    ...
    controller: async (req, res, next) => {
      try {
        ...
        if (generalSetting) {
          return res.send(
            await baseResults(GeneralSetting, "id", req.params, false, [])
          );
        } else {
          res
            .status(404)
            .send({ message: "There is no GeneralSetting with this id" });
        }
      } catch (error) {
        if (process.env.NODE_ENV !== "production") {
          console.log(error);
  ...
  put: {
    ...
    controller: async (req, res, next) => {
      try {
        ...
        const generalSetting = await GeneralSetting.findById(id);
        if (generalSetting) {
          generalSetting.logo = req.files.logo
            ? generateFileName(req.files.logo[0], "generalSetting")
            : generalSetting.logo;
          generalSetting.catalog = req.files.catalog
            ? generateFileName(req.files.catalog[0], "generalSetting")
            : generalSetting.catalog;
          ...
          if (updatedGeneralSetting) {
            ...
            return res
              .status(500)
              .send({ message: "Error in updating GeneralSetting" });
          }
        } else {
          return res
            .status(404)
