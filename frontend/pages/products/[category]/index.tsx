...
import FiltersSection from "../../../Core/Components/FiltersSection";
    ...
    <div>
      ...
      </div>
      {
        <>
          ...
          <div
            className={`w-full  ${
              hotOffers?.length !== 0 ? "mt-8" : "mt-40"
            }  mb-32 flex lg:flex-col flex-row relative`}
          >
            {/* filters */}
            <div className="lg:w-full w-96 h-full p-2">
              <FiltersSection />
            </div>
            ...
            {productList?.length !== 0 ? (
              <>
                <div
                  className={`w-full  relative grid grid-cols-4 xl:grid-cols-3 lg:grid-cols-2 md:grid-cols-1 gap-2`}
                >
                  {productList.map((item) => (
                    <Link
                      href={`/product/${item._id}/${item.name
                        .split(" ")
                        .join("-")}`}
                      className="w-full max-h-[35rem] p-3 border-2 text-white bg-black/50 hover:bg-black hover:scale-100 scale-90 border-white rounded-xl flex flex-col gap-4 items-center justify-center"
                    >
                      <img
                        src={
                          item.image
