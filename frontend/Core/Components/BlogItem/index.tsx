...
const BlogItem = ({item , color , isMain}:any) => {
  return (
    isMain 
    ?
    <div className={`p-4 border border-[#000] rounded-xl flex flex-col gap-2 justify-between w-full  ${color == 'white' ? "text-white border-[#FFF]" :""}`} key={item?.id}>
      ...
      <div className="flex items-center gap-1 text-sm text-[#979797]">
          <p>{new Date(item?.created_date).toLocaleDateString("fa-ir",{year:'numeric',month:'numeric',day:'numeric'})}</p>
          <p>{item?.readingTime}</p>
          <p>{parseInt(item?.seenCount).toLocaleString("fa-ir")} دیدگاه</p>
      </div>
