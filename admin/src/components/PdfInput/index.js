    ...
    <label htmlFor={id} className={`w-full flex-1  z-10 cursor-pointer justify-center  bg-white/10 min-h-max h-full p-6 rounded-md flex flex-col items-center`}>
...

        {
            (
                ...
                ( typeof value !== 'string' ?
                <>
                        <Button className={'z[900]'} onClick={() => window.open(URL.createObjectURL(value))}>
                            دیدن نمونه
                        </Button>
                     ...
                     :
                    <>
                       <embed  src={ApiConfig.domain + value} className='h-full rounded-md' />
                       <p className='text-primary-600 mt-3'>برای تغییر فایل کنید</p>
                       </> )
                
                : 
                <>
                <p className='text-primary-600 text-9xl'><BiSolidFilePdf /></p>
