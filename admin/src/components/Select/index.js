  ...
  useEffect(() => {
    if (search.length >= 2) {
      let list = product
        ? items.filter(
            (item) =>
              item?.productSubCategory?.productMainCategory?.name.indexOf(
                search
              ) !== -1 ||
              item?.productSubCategory?.name.indexOf(search) !== -1 ||
              item?.productCompany?.name.indexOf(search) !== -1
          )
        : items.filter((itm) => itm[keyOfOption].indexOf(search) !== -1);
    ...
    >
      ...
      >
        {hasSearch && (
          <input
            type="search"
            className=" outline-none px-4 py-2 border-b bg-navyLight border-gold3F"
            placeholder="جستجو..."
            value={search}
          ...
          ? ShownItems.map((item, index) => (
              <p
                ...
                onClick={() => {
                  setChoosedOption(
                    product
                      ? `${item.productSubCategory.productMainCategory.name} ${item.productSubCategory.name} ${item.thickness} میل - ${item.productCompany.name} - ${item.city.name}`
                      : keyOfOptionList?.length !== 0
                      ? keyOfOptionList
                          ?.filter((itm) => itm)
                          ?.map(
                            (itm, idx) =>
                              findObjectValue(item, itm?.properties) +
                              (itm?.customText ? itm.customText : "") +
                              (idx == keyOfOptionList?.length - 1 ? "" : " - ")
                          )
                      : item[keyOfOption]
                  );
                  setShowItems(false);
